...
// static parameters:
char output_filename[100] = "data_replicability_gradient_equal_GC_content_seed_";
...
int sequence_matrix[N_REPLICATOR_TYPES][L]; //N*10 matrix for initial (master) sequences: rows->types, columns->sequence loci
...
int master_master_HDs[55];
int master_master_BEs[55];
int master_complement_HDs[100];
int master_complement_BEs[100];
...
void HDs_between_masters(void){  //                      
	...
	for(sp_1st_col = 0; sp_1st_col < N_REPLICATOR_TYPES; sp_1st_col++) 
	{
		for(pair = sp_1st_col; pair < N_REPLICATOR_TYPES; pair++) 
		{
			...
			master_master_HDs[counter]=Hamming_D;
			counter++;
		}
	}
}
...
void BEs_between_masters(void){  //                      
	...
	for(sp_1st_col = 0; sp_1st_col < N_REPLICATOR_TYPES; sp_1st_col++) 
	{
		for(pair = sp_1st_col; pair < N_REPLICATOR_TYPES; pair++) 
		{
			...
			master_master_BEs[counter]=Binding_E;
			counter++;
		}
	}
}

void BEs_between_complementers_and_masters(void){  //                      
	int Binding_E, sp_1st_col, pair, counter=0;	  
	for(sp_1st_col = 0; sp_1st_col < N_REPLICATOR_TYPES; sp_1st_col++) 
	{
		for(pair = 0; pair < N_REPLICATOR_TYPES; pair++) 
		{
			Binding_E=0;
			for(int *x= sequence_matrix[sp_1st_col], *y= complementary_sequence_matrix[pair] + L-1, *x_max= sequence_matrix[sp_1st_col] + L; x != x_max; ++x, --y) Binding_E += BE_lookup[*x][*y];
			master_complement_BEs[counter]=Binding_E;
			counter++;
		}
	}
}
...
{	
	...
	// generate prepared but random sequences containing 5 A or U, and 5 G or C, i.e., with 50% GC content
	for(wf = 0; wf < N_REPLICATOR_TYPES; wf++)
	{
		if(wf==0) // First row of the matrix is certainly not identical to any other row
		{
			for(jed=0;jed<5;jed++) 
			{                       // A G  U  C -> 0,1,2,3
				if(randd() < 0.5) sequence_matrix[wf][jed]=0; 
				else sequence_matrix[wf][jed]=2;
				if(randd() < 0.5) sequence_matrix[wf][5+jed]=1;
				else sequence_matrix[wf][5+jed]=3;				
			}
	...
	BEs_between_masters();
	BEs_between_complementers_and_masters();
...
void define_fitness(int ind)	
{
	...
	int complement_Hd_vec[N_REPLICATOR_TYPES];
	...
	// test for complementaryness:
	comp[ind].complement=-222;
	for(sp=0;sp<N_REPLICATOR_TYPES;sp++)
	{
		complement_Hd_vec[sp]=0;
		for(sqnc=0;sqnc<L;sqnc++)
		{
			if(comp[ind].seq[sqnc]!=complementary_sequence_matrix[sp][sqnc]) complement_Hd_vec[sp]++; //  the number of loci that is different from the master in the given sequence
			if(complement_Hd_vec[sp]>0) break; 
		}	
		if(complement_Hd_vec[sp]==0) 
		{
			comp[ind].complement=sp;
			break; 
		}
	}	
...
int main(int argc, char** argv)
{
	...
	int ik, jk, chbe;
	int co, inner_counter=0, co_counter=0, sum_master_complementary_BEs=0;
	...
	random_integer_seed_variable=1673793449;
	...
	fprintf(parameters, "Complementary master sequences: \n");		
	for(ik=0; ik<N_REPLICATOR_TYPES; ik++) {
		fprintf(parameters, "sp%d HD0 sequence: ",ik);
		for(jk=0;jk<L;jk++) {
			fprintf(parameters, "%d ", complementary_sequence_matrix[ik][jk]);
			if(jk==L-1){
				fprintf(parameters, "\n");
			}
		}
	...
	fprintf(parameters, "%d ", master_master_HDs[0]); // 0-0
	fprintf(parameters, "%d ", master_master_HDs[1]); // 0-1
	fprintf(parameters, "%d ", master_master_HDs[2]); // 0-2
	fprintf(parameters, "%d ", master_master_HDs[3]); // 0-3
	fprintf(parameters, "%d ", master_master_HDs[4]); // 0-4
	fprintf(parameters, "%d ", master_master_HDs[5]); // 0-5
	fprintf(parameters, "%d ", master_master_HDs[6]); // 0-6
	fprintf(parameters, "%d ", master_master_HDs[7]); // 0-7
	fprintf(parameters, "%d ", master_master_HDs[8]); // 0-8
	fprintf(parameters, "%d ", master_master_HDs[9]); // 0-9
	fprintf(parameters, "\n");
	
	fprintf(parameters, "sp1 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[1]);  // 1-0
	fprintf(parameters, "%d ", master_master_HDs[10]); // 1-1
	fprintf(parameters, "%d ", master_master_HDs[11]); // 1-2
	fprintf(parameters, "%d ", master_master_HDs[12]); // 1-3
	fprintf(parameters, "%d ", master_master_HDs[13]); // 1-4
	fprintf(parameters, "%d ", master_master_HDs[14]); // 1-5
	fprintf(parameters, "%d ", master_master_HDs[15]); // 1-6
	fprintf(parameters, "%d ", master_master_HDs[16]); // 1-7
	fprintf(parameters, "%d ", master_master_HDs[17]); // 1-8
	fprintf(parameters, "%d ", master_master_HDs[18]);	// 1-9	
	fprintf(parameters, "\n");		

	fprintf(parameters, "sp2 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[2]);  // 2-0
	fprintf(parameters, "%d ", master_master_HDs[11]); // 2-1
	fprintf(parameters, "%d ", master_master_HDs[19]); // 2-2
	fprintf(parameters, "%d ", master_master_HDs[20]); // 2-3
	fprintf(parameters, "%d ", master_master_HDs[21]); // 2-4
	fprintf(parameters, "%d ", master_master_HDs[22]); // 2-5
	fprintf(parameters, "%d ", master_master_HDs[23]); // 2-6
	fprintf(parameters, "%d ", master_master_HDs[24]); // 2-7
	fprintf(parameters, "%d ", master_master_HDs[25]); // 2-8
	fprintf(parameters, "%d ", master_master_HDs[26]);	// 2-9	
	fprintf(parameters, "\n");	
	
	fprintf(parameters, "sp3 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[3]);  // 3-0
	fprintf(parameters, "%d ", master_master_HDs[12]); // 3-1
	fprintf(parameters, "%d ", master_master_HDs[20]); // 3-2
	fprintf(parameters, "%d ", master_master_HDs[27]); // 3-3
	fprintf(parameters, "%d ", master_master_HDs[28]); // 3-4
	fprintf(parameters, "%d ", master_master_HDs[29]); // 3-5
	fprintf(parameters, "%d ", master_master_HDs[30]); // 3-6
	fprintf(parameters, "%d ", master_master_HDs[31]); // 3-7
	fprintf(parameters, "%d ", master_master_HDs[32]); // 3-8
	fprintf(parameters, "%d ", master_master_HDs[33]);	// 3-9	
	fprintf(parameters, "\n");		
	
	fprintf(parameters, "sp4 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[4]);  // 4-0
	fprintf(parameters, "%d ", master_master_HDs[13]); // 4-1
	fprintf(parameters, "%d ", master_master_HDs[21]); // 4-2
	fprintf(parameters, "%d ", master_master_HDs[28]); // 4-3
	fprintf(parameters, "%d ", master_master_HDs[34]); // 4-4
	fprintf(parameters, "%d ", master_master_HDs[35]); // 4-5
	fprintf(parameters, "%d ", master_master_HDs[36]); // 4-6
	fprintf(parameters, "%d ", master_master_HDs[37]); // 4-7
	fprintf(parameters, "%d ", master_master_HDs[38]); // 4-8
	fprintf(parameters, "%d ", master_master_HDs[39]);	// 4-9	
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp5 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[5]);  // 5-0
	fprintf(parameters, "%d ", master_master_HDs[14]); // 5-1
	fprintf(parameters, "%d ", master_master_HDs[22]); // 5-2
	fprintf(parameters, "%d ", master_master_HDs[29]); // 5-3
	fprintf(parameters, "%d ", master_master_HDs[35]); // 5-4
	fprintf(parameters, "%d ", master_master_HDs[40]); // 5-5
	fprintf(parameters, "%d ", master_master_HDs[41]); // 5-6
	fprintf(parameters, "%d ", master_master_HDs[42]); // 5-7
	fprintf(parameters, "%d ", master_master_HDs[43]); // 5-8
	fprintf(parameters, "%d ", master_master_HDs[44]);	// 5-9	
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp6 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[6]);  // 6-0
	fprintf(parameters, "%d ", master_master_HDs[15]); // 6-1
	fprintf(parameters, "%d ", master_master_HDs[23]); // 6-2
	fprintf(parameters, "%d ", master_master_HDs[30]); // 6-3
	fprintf(parameters, "%d ", master_master_HDs[36]); // 6-4
	fprintf(parameters, "%d ", master_master_HDs[41]); // 6-5
	fprintf(parameters, "%d ", master_master_HDs[45]); // 6-6
	fprintf(parameters, "%d ", master_master_HDs[46]); // 6-7
	fprintf(parameters, "%d ", master_master_HDs[47]); // 6-8
	fprintf(parameters, "%d ", master_master_HDs[48]);	// 6-9	
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp7 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[7]);  // 7-0
	fprintf(parameters, "%d ", master_master_HDs[16]); // 7-1
	fprintf(parameters, "%d ", master_master_HDs[24]); // 7-2
	fprintf(parameters, "%d ", master_master_HDs[31]); // 7-3
	fprintf(parameters, "%d ", master_master_HDs[37]); // 7-4
	fprintf(parameters, "%d ", master_master_HDs[42]); // 7-5
	fprintf(parameters, "%d ", master_master_HDs[46]); // 7-6
	fprintf(parameters, "%d ", master_master_HDs[49]); // 7-7
	fprintf(parameters, "%d ", master_master_HDs[50]); // 7-8
	fprintf(parameters, "%d ", master_master_HDs[51]);	// 7-9	
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp8 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[8]);  // 8-0
	fprintf(parameters, "%d ", master_master_HDs[17]); // 8-1
	fprintf(parameters, "%d ", master_master_HDs[25]); // 8-2
	fprintf(parameters, "%d ", master_master_HDs[32]); // 8-3
	fprintf(parameters, "%d ", master_master_HDs[38]); // 8-4
	fprintf(parameters, "%d ", master_master_HDs[43]); // 8-5
	fprintf(parameters, "%d ", master_master_HDs[47]); // 8-6
	fprintf(parameters, "%d ", master_master_HDs[50]); // 8-7
	fprintf(parameters, "%d ", master_master_HDs[52]); // 8-8
	fprintf(parameters, "%d ", master_master_HDs[53]);	// 8-9	
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp9 HDs: ");
	fprintf(parameters, "%d ", master_master_HDs[9]);  // 9-0
	fprintf(parameters, "%d ", master_master_HDs[18]); // 9-1
	fprintf(parameters, "%d ", master_master_HDs[26]); // 9-2
	fprintf(parameters, "%d ", master_master_HDs[33]); // 9-3
	fprintf(parameters, "%d ", master_master_HDs[39]); // 9-4
	fprintf(parameters, "%d ", master_master_HDs[44]); // 9-5
	fprintf(parameters, "%d ", master_master_HDs[48]); // 9-6
	fprintf(parameters, "%d ", master_master_HDs[51]); // 9-7
	fprintf(parameters, "%d ", master_master_HDs[53]); // 9-8
	fprintf(parameters, "%d ", master_master_HDs[54]);	// 9-9	
	fprintf(parameters, "\n");

	for(chbe=0; chbe<55; chbe++) 
	{
		sum_of_master_HDs+=master_master_HDs[chbe];
	}
	fprintf(parameters, "mean Hamming-distances between masters: %f\n", (double)sum_of_master_HDs/(double)45.0);	
	
	fprintf(parameters, "Hamming-distances between master and complementary sequences: \n");
	fprintf(parameters, "sp0 HDs : ");
	for(co=0; co<100; co++)	
	{
	 	fprintf(parameters, "%d ", master_complement_HDs[co]); 
		if(co_counter++>=9) 
		{
		 	inner_counter++;
		 	fprintf(parameters, "\n");
		 	if(inner_counter<10) fprintf(parameters, "sp%d HDs : ",inner_counter);
		 	co_counter=0;	
		}
	}
	...
	fprintf(parameters, "%d ", master_master_BEs[0]); // 0-0
	fprintf(parameters, "%d ", master_master_BEs[1]); // 0-1
	fprintf(parameters, "%d ", master_master_BEs[2]); // 0-2
	fprintf(parameters, "%d ", master_master_BEs[3]); // 0-3
	fprintf(parameters, "%d ", master_master_BEs[4]); // 0-4
	fprintf(parameters, "%d ", master_master_BEs[5]); // 0-5
	fprintf(parameters, "%d ", master_master_BEs[6]); // 0-6
	fprintf(parameters, "%d ", master_master_BEs[7]); // 0-7
	fprintf(parameters, "%d ", master_master_BEs[8]); // 0-8
	fprintf(parameters, "%d ", master_master_BEs[9]); // 0-9
	fprintf(parameters, "  mean BE of sp0: %f", 
	(double)(master_master_BEs[0]+master_master_BEs[1]+master_master_BEs[2]+master_master_BEs[3]+master_master_BEs[4]+master_master_BEs[5]+master_master_BEs[6]+master_master_BEs[7]+master_master_BEs[8]+master_master_BEs[9])/(double)10.0);
	fprintf(parameters, "\n");
	
	fprintf(parameters, "sp1 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[1]);  // 1-0
	fprintf(parameters, "%d ", master_master_BEs[10]); // 1-1
	fprintf(parameters, "%d ", master_master_BEs[11]); // 1-2
	fprintf(parameters, "%d ", master_master_BEs[12]); // 1-3
	fprintf(parameters, "%d ", master_master_BEs[13]); // 1-4
	fprintf(parameters, "%d ", master_master_BEs[14]); // 1-5
	fprintf(parameters, "%d ", master_master_BEs[15]); // 1-6
	fprintf(parameters, "%d ", master_master_BEs[16]); // 1-7
	fprintf(parameters, "%d ", master_master_BEs[17]); // 1-8
	fprintf(parameters, "%d ", master_master_BEs[18]);	// 1-9	
	fprintf(parameters, "  mean BE of sp1: %f", 
	(double)(master_master_BEs[1]+master_master_BEs[10]+master_master_BEs[11]+master_master_BEs[12]+master_master_BEs[13]+master_master_BEs[14]+master_master_BEs[15]+master_master_BEs[16]+master_master_BEs[17]+master_master_BEs[18])/(double)10.0);
	fprintf(parameters, "\n");

	fprintf(parameters, "sp2 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[2]);  // 2-0
	fprintf(parameters, "%d ", master_master_BEs[11]); // 2-1
	fprintf(parameters, "%d ", master_master_BEs[19]); // 2-2
	fprintf(parameters, "%d ", master_master_BEs[20]); // 2-3
	fprintf(parameters, "%d ", master_master_BEs[21]); // 2-4
	fprintf(parameters, "%d ", master_master_BEs[22]); // 2-5
	fprintf(parameters, "%d ", master_master_BEs[23]); // 2-6
	fprintf(parameters, "%d ", master_master_BEs[24]); // 2-7
	fprintf(parameters, "%d ", master_master_BEs[25]); // 2-8
	fprintf(parameters, "%d ", master_master_BEs[26]);	// 2-9	
	fprintf(parameters, "  mean BE of sp2: %f", 
	(double)(master_master_BEs[2]+master_master_BEs[11]+master_master_BEs[19]+master_master_BEs[20]+master_master_BEs[21]+master_master_BEs[22]+master_master_BEs[23]+master_master_BEs[24]+master_master_BEs[25]+master_master_BEs[26])/(double)10.0);
	fprintf(parameters, "\n");
	
	fprintf(parameters, "sp3 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[3]);  // 3-0
	fprintf(parameters, "%d ", master_master_BEs[12]); // 3-1
	fprintf(parameters, "%d ", master_master_BEs[20]); // 3-2
	fprintf(parameters, "%d ", master_master_BEs[27]); // 3-3
	fprintf(parameters, "%d ", master_master_BEs[28]); // 3-4
	fprintf(parameters, "%d ", master_master_BEs[29]); // 3-5
	fprintf(parameters, "%d ", master_master_BEs[30]); // 3-6
	fprintf(parameters, "%d ", master_master_BEs[31]); // 3-7
	fprintf(parameters, "%d ", master_master_BEs[32]); // 3-8
	fprintf(parameters, "%d ", master_master_BEs[33]);	// 3-9	
	fprintf(parameters, "  mean BE of sp3: %f", 
	(double)(master_master_BEs[3]+master_master_BEs[12]+master_master_BEs[20]+master_master_BEs[27]+master_master_BEs[28]+master_master_BEs[29]+master_master_BEs[30]+master_master_BEs[31]+master_master_BEs[32]+master_master_BEs[33])/(double)10.0);
	fprintf(parameters, "\n");	
	
	fprintf(parameters, "sp4 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[4]);  // 4-0
	fprintf(parameters, "%d ", master_master_BEs[13]); // 4-1
	fprintf(parameters, "%d ", master_master_BEs[21]); // 4-2
	fprintf(parameters, "%d ", master_master_BEs[28]); // 4-3
	fprintf(parameters, "%d ", master_master_BEs[34]); // 4-4
	fprintf(parameters, "%d ", master_master_BEs[35]); // 4-5
	fprintf(parameters, "%d ", master_master_BEs[36]); // 4-6
	fprintf(parameters, "%d ", master_master_BEs[37]); // 4-7
	fprintf(parameters, "%d ", master_master_BEs[38]); // 4-8
	fprintf(parameters, "%d ", master_master_BEs[39]);	// 4-9	
	fprintf(parameters, "  mean BE of sp4: %f", 
	(double)(master_master_BEs[4]+master_master_BEs[13]+master_master_BEs[21]+master_master_BEs[28]+master_master_BEs[34]+master_master_BEs[35]+master_master_BEs[36]+master_master_BEs[37]+master_master_BEs[38]+master_master_BEs[39])/(double)10.0);
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp5 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[5]);  // 5-0
	fprintf(parameters, "%d ", master_master_BEs[14]); // 5-1
	fprintf(parameters, "%d ", master_master_BEs[22]); // 5-2
	fprintf(parameters, "%d ", master_master_BEs[29]); // 5-3
	fprintf(parameters, "%d ", master_master_BEs[35]); // 5-4
	fprintf(parameters, "%d ", master_master_BEs[40]); // 5-5
	fprintf(parameters, "%d ", master_master_BEs[41]); // 5-6
	fprintf(parameters, "%d ", master_master_BEs[42]); // 5-7
	fprintf(parameters, "%d ", master_master_BEs[43]); // 5-8
	fprintf(parameters, "%d ", master_master_BEs[44]);	// 5-9	
	fprintf(parameters, "  mean BE of sp5: %f", 
	(double)(master_master_BEs[5]+master_master_BEs[14]+master_master_BEs[22]+master_master_BEs[29]+master_master_BEs[35]+master_master_BEs[40]+master_master_BEs[41]+master_master_BEs[42]+master_master_BEs[43]+master_master_BEs[44])/(double)10.0);
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp6 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[6]);  // 6-0
	fprintf(parameters, "%d ", master_master_BEs[15]); // 6-1
	fprintf(parameters, "%d ", master_master_BEs[23]); // 6-2
	fprintf(parameters, "%d ", master_master_BEs[30]); // 6-3
	fprintf(parameters, "%d ", master_master_BEs[36]); // 6-4
	fprintf(parameters, "%d ", master_master_BEs[41]); // 6-5
	fprintf(parameters, "%d ", master_master_BEs[45]); // 6-6
	fprintf(parameters, "%d ", master_master_BEs[46]); // 6-7
	fprintf(parameters, "%d ", master_master_BEs[47]); // 6-8
	fprintf(parameters, "%d ", master_master_BEs[48]);	// 6-9	
	fprintf(parameters, "  mean BE of sp6: %f", 
	(double)(master_master_BEs[6]+master_master_BEs[15]+master_master_BEs[23]+master_master_BEs[30]+master_master_BEs[36]+master_master_BEs[41]+master_master_BEs[45]+master_master_BEs[46]+master_master_BEs[47]+master_master_BEs[48])/(double)10.0);
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp7 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[7]);  // 7-0
	fprintf(parameters, "%d ", master_master_BEs[16]); // 7-1
	fprintf(parameters, "%d ", master_master_BEs[24]); // 7-2
	fprintf(parameters, "%d ", master_master_BEs[31]); // 7-3
	fprintf(parameters, "%d ", master_master_BEs[37]); // 7-4
	fprintf(parameters, "%d ", master_master_BEs[42]); // 7-5
	fprintf(parameters, "%d ", master_master_BEs[46]); // 7-6
	fprintf(parameters, "%d ", master_master_BEs[49]); // 7-7
	fprintf(parameters, "%d ", master_master_BEs[50]); // 7-8
	fprintf(parameters, "%d ", master_master_BEs[51]);	// 7-9	
	fprintf(parameters, "  mean BE of sp7: %f", 
	(double)(master_master_BEs[7]+master_master_BEs[16]+master_master_BEs[24]+master_master_BEs[31]+master_master_BEs[37]+master_master_BEs[42]+master_master_BEs[46]+master_master_BEs[49]+master_master_BEs[50]+master_master_BEs[51])/(double)10.0);
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp8 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[8]);  // 8-0
	fprintf(parameters, "%d ", master_master_BEs[17]); // 8-1
	fprintf(parameters, "%d ", master_master_BEs[25]); // 8-2
	fprintf(parameters, "%d ", master_master_BEs[32]); // 8-3
	fprintf(parameters, "%d ", master_master_BEs[38]); // 8-4
	fprintf(parameters, "%d ", master_master_BEs[43]); // 8-5
	fprintf(parameters, "%d ", master_master_BEs[47]); // 8-6
	fprintf(parameters, "%d ", master_master_BEs[50]); // 8-7
	fprintf(parameters, "%d ", master_master_BEs[52]); // 8-8
	fprintf(parameters, "%d ", master_master_BEs[53]);	// 8-9	
	fprintf(parameters, "  mean BE of sp8: %f", 
	(double)(master_master_BEs[8]+master_master_BEs[17]+master_master_BEs[25]+master_master_BEs[32]+master_master_BEs[38]+master_master_BEs[43]+master_master_BEs[47]+master_master_BEs[50]+master_master_BEs[52]+master_master_BEs[53])/(double)10.0);
	fprintf(parameters, "\n");	

	fprintf(parameters, "sp9 BEs: ");
	fprintf(parameters, "%d ", master_master_BEs[9]);  // 9-0
	fprintf(parameters, "%d ", master_master_BEs[18]); // 9-1
	fprintf(parameters, "%d ", master_master_BEs[26]); // 9-2
	fprintf(parameters, "%d ", master_master_BEs[33]); // 9-3
	fprintf(parameters, "%d ", master_master_BEs[39]); // 9-4
	fprintf(parameters, "%d ", master_master_BEs[44]); // 9-5
	fprintf(parameters, "%d ", master_master_BEs[48]); // 9-6
	fprintf(parameters, "%d ", master_master_BEs[51]); // 9-7
	fprintf(parameters, "%d ", master_master_BEs[53]); // 9-8
	fprintf(parameters, "%d ", master_master_BEs[54]);	// 9-9	
	fprintf(parameters, "  mean BE of sp9: %f", 
	(double)(master_master_BEs[9]+master_master_BEs[18]+master_master_BEs[26]+master_master_BEs[33]+master_master_BEs[39]+master_master_BEs[44]+master_master_BEs[48]+master_master_BEs[51]+master_master_BEs[53]+master_master_BEs[54])/(double)10.0);
	fprintf(parameters, "\n");

	fprintf(parameters, "Binding energies between master and complementary sequences: \n");
	fprintf(parameters, "sp0 BEs : ");
	inner_counter=0;
	for(co=0; co<100; co++)	
	{
	 	sum_master_complementary_BEs+=master_complement_BEs[co];
	 	fprintf(parameters, "%d ", master_complement_BEs[co]); 
		if(co_counter++>=9) 
		{
		 	fprintf(parameters, "  mean BE of sp%d: %f", inner_counter, (double)sum_master_complementary_BEs/(double)10.0);
		 	sum_master_complementary_BEs=0;
		 	inner_counter++;
		 	fprintf(parameters, "\n");
		 	if(inner_counter<10) fprintf(parameters, "sp%d BEs : ",inner_counter);
		 	co_counter=0;	
		}
	}
	...
	while(replication_counter<MAX_REPLICATIONS){ // The main loop
		...
		if(event==0)  
		{
			...
			(comp[simplex_lookup[a_one]].master==comp[simplex_lookup[z_one]].complement || // plus-minus strand of a given species can associate
			comp[simplex_lookup[z_one]].master==comp[simplex_lookup[a_one]].complement ||  
			(comp[simplex_lookup[z_one]].master==-888 && comp[simplex_lookup[z_one]].complement==-222) ||// if one of the sequences is nor plus neither 
			(comp[simplex_lookup[a_one]].master==-888 && comp[simplex_lookup[a_one]].complement==-222))) // minus strand of a master, i.e., mutant, 
			{ 																							 // association is also permitted			
...
// compile: gcc replicability_gradient_equal_GC_content.c -lm -Wall -Ofast -o replicability_gradient
// run: nohup ./replicability_gradient & 

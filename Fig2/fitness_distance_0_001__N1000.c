...
#define SEQ_NUM (1000) // population size, ie., the number of sequences {1000, 10000, 100000}
...
// static parameters:
char output_filename[100] = "data_delta_0_001__N1000_seed_";
...
int main(int argc, char** argv)
{
	...
	fclose(file);
	FILE *surv = NULL;
	surv = fopen("survival_stat__delta_0_001__N1000.txt", "a");
	fprintf(surv, "%.1f, ", ((double)surv_num)/((double)10));
	fclose(surv);
	FILE *sim_dup_file = NULL;
	sim_dup_file = fopen("simplex_duplex_ratio_mean__delta_0_001__N1000.txt", "a");
	fprintf(sim_dup_file, "%f, ", ((double)sum_simp_dup)/((double)sum_simp_dup_counter));
	fclose(sim_dup_file);	
	FILE *sum_master_file = NULL;
	sum_master_file = fopen("mean_sum_of_all_masters__delta_0_001__N1000.txt", "a");
	fprintf(sum_master_file, "%f, ", ((double)add_rel_sum_of_all_masters)/((double)sum_simp_dup_counter));
	fclose(sum_master_file);
	return(0);
}//main end

// compile: gcc fitness_distance_0_001__N1000.c -lm -Wall -Ofast -o delta_0_001__N1000
// run: nohup ./delta_0_001__N1000 &           
